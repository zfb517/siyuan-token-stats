...
- **三重存储备份**：主存储（`data/storage/`）+ 插件目录备份（`data/plugins/`）+ localStorage，防止插件更新或异常时数据丢失
...
- **conf.json 扫描**：插件通过 `window.siyuan.config.ai` 实时读取思源 AI 配置（含 providers、models），用于将块 ID 解析为真实模型名。此操作只读取内存中的配置，不修改 conf.json 文件
