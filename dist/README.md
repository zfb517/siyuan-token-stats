...
- **「立即同步」按钮的权限说明**：该按钮调用思源内核接口 `/api/sync/now` 尝试触发一次云同步，再把其他端数据合并到本地。它**依赖思源自身的同步引擎**，需同时满足：① 思源「设置 - 关于 - 云端」已配置并启用云同步（存储服务已登录）；② 当前环境允许插件调用该内核接口（需携带思源 API token 鉴权）。**插件无法授予思源云同步权限**——若思源未启用云同步 / 未登录 / 拒绝该调用，按钮不会触发同步，只会合并本地磁盘上已有的数据，并在提示中明确给出失败原因（如思源返回的错误信息）。最可靠的跨端同步方式：先在思源「设置 - 关于 - 云端」手动触发一次云同步，再点击「立即同步」合并。
