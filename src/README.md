...
4. **全局费用限额**：在设置中填写「全局费用限额」（金额）、「全局费用提醒阈值 (%)」，并可单独设置「全局费用重置周期」（默认每月，独立于 Token 限额周期，支持每月 / 每日 / 永不）；如需把第三方平台的历史花费计入预算起点，可填「全局已用费用校准」。达到预算上限时底部弹出提醒（需先在「费用估算配置」中设好模型单价，否则费用为 0）
