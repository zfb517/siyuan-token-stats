...
- **费用估算**：在插件设置中按模型配置输入 / 输出单价（如 `deepseek-v4-flash` 输入 ¥0.001/千 token、输出 ¥0.004/千 token），也支持配置「资源包」——一个资源包可涵盖多个模型、包内模型共用同一单价（适合某套餐内含多个模型的场景）；仪表盘自动按模型汇总估算总费用；汇总卡片、最近请求记录表、模型用量分布均展示费用（未配置单价时显示"未配置")
...
6. **费用估算配置**：在插件设置中点击「费用估算配置」
   - 选择币种（¥ 人民币 / $ 美元）
   - **按模型单价**：为每个模型添加输入单价与输出单价（单位：每千 token 的费用，例如 DeepSeek V3 输入 ¥0.001、输出 ¥0.004）。模型名填写时统一转为小写（与统计中的模型匹配）
